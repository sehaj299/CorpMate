...
const AddDepartmentDialog: React.FC<AddDepartmentDialogProps> = ({
  ...
  const handleSave = async () => {
    try {
      const jwt = localStorage.getItem("jwt"); // Check if department name already exists

      //   const checkResponse = await axios.get(
      //     `http://localhost:1337/api/departments?name=${departmentName}`,

      //     { headers: { Authorization: `Bearer ${jwt}` } }
      //   );

      //   const existingDepartment = checkResponse.data[0];

      //   if (existingDepartment) {
      //     console.log(`Department name '${departmentName}' already exists`);
...
const TabsForcedScroll = () => {
  ...
  const toggleAddUserDrawer = () => setIsAddDepartmentDialogOpen(!isAddDepartmentDialogOpen);
